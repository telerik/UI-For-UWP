...
namespace Telerik.Data.Core.Layouts
{
    internal class CompactLayout : BaseLayout
    {
        ...
        public override IRenderInfo RenderInfo
        {
            get
            {
                if (this.renderInfo == null)
                {
                    this.renderInfo = new IndexStorage(this.totalCount, this.DefaultItemLength);
                }
        ...
        public override int TotalLineCount
        {
        ...
        public override void Expand(object item)
        {
            ...
            if (isCollapsed)
            {
                ...
                if (groupInfo.IsVisible())
                {
                    ...
                    ////int layoutSlot = this.GetVisibleSlot(slot);
        ...
        internal override AddRemoveLayoutResult AddItem(object changedItem, object addRemoveItem, int addRemoveItemIndex)
        {
            ...
            this.RenderInfo.InsertRange(slot, null, count);
        ...
        internal override double IndexFromPhysicalOffset(double physicalOffset, bool includeCollapsed = false)
        {
        ...
        internal LayoutInfo GenerateLayoutInfo(ItemInfo itemInfo, GroupInfo parentGroupInfo, int visibleLine, int childIndex)
        {
            LayoutInfo info = new LayoutInfo();
            info.Indent = this.GetIndent(itemInfo, parentGroupInfo);
            info.Line = visibleLine;
            info.ChildLine = childIndex;
            info.LineSpan = 1;
            info.LevelSpan = 1;
            info.SpansThroughCells = true;
            info.Level = this.GetLayoutLevel(itemInfo, parentGroupInfo);
        ...
        protected override void SetItemsSourceOverride(IReadOnlyList<object> source, bool restoreCollapsed)
        {
            ...
            foreach (var strategy in this.LayoutStrategies)
            {
                slotsCount += strategy.CalculateAppendedSlotsCount(this, slotsCount);
            }

            this.totalCount = this.visibleLineCount = slotsCount;
